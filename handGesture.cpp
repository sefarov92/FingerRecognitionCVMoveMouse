...
void HandGesture::addNumberToImg(MyImage *m){
	...
	//new mouse commnad starts here
	/*
	bool indexfin = false;
	bool middlefin = false;
	String position;
	if (numbers2Display.size()<=3)
	{
		for (int i = 0; i < numbers2Display.size(); i++)
		{
			...
			if (numbers2Display.size() == 0)
			{
				...
				SendInput(1, &ip, sizeof(INPUT));
				putText(m->src, position, Point(400, 50), 1, 2, CV_RGB(255, 0, 0), 2, 8);
				//SetCursorPos(1366 / 400 * (middle.x - 100), 800 / 200 * (middle.y - 200));
			}
			else if (numbers2Display.size() == 1)
			{
				position == "Left Click";
					putText(m->src, position, Point(400, 50), 1, 2, CV_RGB(255, 0, 0), 2, 8);
			...
			else if (numbers2Display.size() == 2)
			{
			...
			else putText(m->src, "power", Point(400, 50), 1, 2, CV_RGB(255, 0, 0), 2, 8);
...
//		position = "Left Click";
	...
	//new mouse commnad ends here hope works
}
...
float HandGesture::getAngle(Point s, Point f, Point e){
	...
	angle=angle*180/PI;
	//SetCursorPos((s.x - f.x)*(e.x - f.x),(s.y - f.y)*(e.y - f.y));
	
	return angle;
}
//new angle class
float HandGesture::getAngle1(Point s, Point f, Point e){

	//new angle code
	int ang;
	if (s.x == f.x == e.x)
		return 0;
	ang = int(atan((s.x - f.x)*(e.x - f.x) + (s.y - f.y)*(e.y - f.y)) * 180 / 3.14);
	if (ang < 0)
		return ang + 180;
	else
		return ang;
	//new angle code end here

}//new angle class end here
...
void HandGesture::checkForOneFinger(MyImage *m){
	...
	while( d!=contours[cIdx].end() ) {
		...
		if(v.y<highestP.y){
			...
			SetCursorPos(1366 / 300 * (v.x - 100), 800 / 100 * (v.y - 200));
		}
		d++;	
	}int n=0;
	d=hullP[cIdx].begin();
